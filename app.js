...
// *********************************************************** //
const animes = require('./public/data/anime.json')
...
router.get('./public/data/anime.json', function(req, res, next) {
...
/* ************************
   ...
   ************************ */
// this route loads in the animes into the Anime collection
// or updates the animes if it is not a new collection

app.get('/upsertDB',
    async(req, res, next) => {
        //await Anime.deleteMany({})
        for (anime of animes) {
            const { Name, Japanese, Score, Genres } = anime;
            await Anime.findOneAndUpdate({ Name, Japanese, Score, Genres }, anime, { upsert: true })
        }
        const num = await Anime.find({}).count();
        res.send("data uploaded: " + num)
    }
)


app.post('/animes/byName',
    // show list of animes with a name
    async(req, res, next) => {
        const { name } = req.body;
        var regex = new RegExp(name, "gi")
        const animes = await Anime.find({ Name: regex })
